alter table public.users
  add column if not exists action_permissions text[] not null default '{}'::text[];

update public.users
set action_permissions = array[
  'duzenleme',
  'pasife_alma'
]::text[]
where role = 'personel'
  and (
    action_permissions is null
    or coalesce(array_length(action_permissions, 1), 0) = 0
  );
...
grant execute on function public.admin_create_personnel(text, text, text, text, text[], text[]) to authenticated;
