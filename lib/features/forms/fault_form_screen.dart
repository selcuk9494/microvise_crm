...
final faultFormCustomersProvider = FutureProvider<List<_FaultCustomerOption>>((
  ref,
) async {
  ...
  while (true) {
    ...
    final batch = (rows as List)
        .map(
          (row) => _FaultCustomerOption.fromJson(row as Map<String, dynamic>),
        )
        .toList(growable: false);
    items.addAll(batch);
  ...
  Future<void> _print(FaultFormRecord record) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          ok
              ? 'Arıza formu çıktısı hazırlandı.'
              : 'Bu platformda çıktı açılamadı.',
        ),
      ),
    );
  }
  ...
  List<FaultFormRecord> _filter(List<FaultFormRecord> input) {
    ...
    return input
        .where((r) {
          ...
          if (qCustomer.isNotEmpty &&
              !_sortKey(r.customerName).contains(qCustomer)) {
            return false;
          }
          ...
          Wrap(
            ...
            children: [
              ...
              OutlinedButton.icon(
                ...
                label: Text(
                  _showPassive ? 'Pasifleri Gizle' : 'Pasifleri Göster',
                ),
              ),
            ],
          ),
            ...
            itemBuilder: (context, index) {
              ...
              return _FaultRecordCard(
                ...
                onUploadDocument: () => handleFormDocumentAction(
                  ...
                  recordId: record.id,
                  defaultFilename:
                      'ariza-formu-${safeFormDocumentFilePart(record.customerName)}',
                  onChanged: () => ref.invalidate(faultFormsProvider),
                ),
  ...
  Future<void> _setActive(FaultFormRecord record, bool active) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            active ? 'Form aktifleştirildi.' : 'Form pasife alındı.',
          ),
        ),
      );
    } catch (e) {
...
class _FaultFormDialogState extends ConsumerState<_FaultFormDialog> {
  ...
  void initState() {
    ...
    _customerPickerController = TextEditingController(
    ...
    _customerAddressController = TextEditingController(
      text: initial?.customerAddress ?? '',
    );
    _customerTaxOfficeController = TextEditingController(
      text: initial?.customerTaxOffice ?? '',
    );
    _customerVknController = TextEditingController(
      text: initial?.customerVkn ?? '',
    );

    _deviceBrandModelController = TextEditingController(
      text: initial?.deviceBrandModel ?? '',
    );
    final currentBrandModel = _deviceBrandModelController.text.trim();
    _brandModelPreset = _brandModelOptions.contains(currentBrandModel)
        ? currentBrandModel
        : null;
    _companyCodeController = TextEditingController(
      text: initial?.companyCodeAndRegistry ?? '',
    );
    _okcApprovalController = TextEditingController(
      text: initial?.okcApprovalDateAndNumber ?? '',
    );
    _faultDateTimeController = TextEditingController(
      text:
          initial?.faultDateTimeText ??
          DateFormat('dd.MM.yyyy HH:mm', 'tr_TR').format(DateTime.now()),
    );
    _faultDescriptionController = TextEditingController(
      text: initial?.faultDescription ?? '',
    );
    _lastZDateController = TextEditingController(
      text: DateFormat('dd.MM.yyyy', 'tr_TR').format(_lastZReportDate!),
    );
    _lastZNoController = TextEditingController(
      text: initial?.lastZReportNo ?? '',
    );
    _totalRevenueController = TextEditingController(
      text: initial?.totalRevenue ?? '',
    );
  ...
  Future<void> _pickLastZDate() async {
    ...
    _lastZDateController.text = DateFormat(
      'dd.MM.yyyy',
      'tr_TR',
    ).format(picked);
  }
                  ...
                  data: (customers) {
                    ...
                    return Column(
                      ...
                      children: [
                        Row(
                          children: <Widget>[
                            Expanded(child: _customerPickerField(customers)),
                            const Gap(8),
                            OutlinedButton(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      TextFormField(
                        ...
                        decoration: const InputDecoration(
                          labelText: "Onay Belgesi Tarih ve No'su",
                        ),
                      ),
                      const Gap(10),
                      TextFormField(
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final payload = <String, dynamic>{
        ...
        'form_date':
            (widget.isEdit ? widget.initialRecord!.formDate : DateTime.now())
                .toIso8601String(),
        ...
        'okc_approval_date_and_number':
            _okcApprovalController.text.trim().isEmpty
            ? null
        ...
        'last_z_report_date': _lastZReportDate?.toIso8601String(),
...
class _FaultCustomerPickerDialogState
    extends State<_FaultCustomerPickerDialog> {
